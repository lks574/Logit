...
- **클라이밍 전용 폼 + 맛집/콘서트 정합**: (①) 클라이밍이 헬스 SetRepForm(부위·중량)에 안 맞던 문제 → 전용 `ClimbingForm`(RecordFormScreen에서 캠핑처럼 분기). 스타일(볼더링/리드/톱로프) · 루트 목록(그레이드·시도·완등) · 완등 요약. `src/lib/climbing.ts`(parseRoutes·cleanRoutes·climbSummary), 저장 `fields.루트`(JSON)·`스타일`. Detail에 루트 breakdown. (②) 맛집은 outing이지만 멀티데이 UI 제거(단일 방문). (③) 콘서트는 spectate 라벨만 공연명·아티스트로(저장 키 작품/출연진 유지). 웹 검증: 클라이밍 V4 완등 1/1·Detail 루트, 맛집 멀티데이 없음, 콘서트 라벨 확인.
- **유산소 종목 프로파일(런닝·자전거·수영 분화)**: 한 EnduranceForm이 진입 활동으로 프로파일을 골라 UI·칼로리를 바꿈(`src/data/endurance.ts`, MatchForm sports 패턴). 런닝=km·km/h·고도·ACSM 칼로리, 자전거=km·km/h·고도·MET 칼로리, 수영=**m·100m 페이스·고도 없음·영법(자유형/배영/평영/접영)·MET 칼로리**. 칼로리 종목별 분화: `src/lib/calories.ts`에 `cyclingCalories`(속도구간 MET)·`swimCalories`(영법×강도 MET) 추가. 저장 스키마 additive(`페이스`·`영법` 키), 마이그레이션 불필요. 웹 검증: 수영 1500m/30분→페이스 2:00/100m·자유형 210kcal→접영 385kcal, Detail 렌더 확인.
