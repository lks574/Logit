...
- **회원(로컬 프로필)**: 설정 상단 카드 → ProfileEdit(이름·이메일 편집). 스토어 `profile`에 저장·영속, 아바타 이니셜은 이름에서 파생. 인증/백엔드 없음(오프라인 로컬만).
...
- **선택 데이터 정합**: 폼별 핵심 필드 필수 검증 추가(빈 저장 차단 + Alert). 단위/형식 강제는 없음(자유 입력 "5.2km"/"27:12" 유지). MatchForm 점수/슬롯은 편집 가능하나 스포츠별 스키마는 라벨 기반.
  - 필수 조건: Endurance=거리|시간, SetRep=부위|총볼륨|세트, Match=스코어|상대|슬롯, Spectate=작품명, Free=시간|메모.
