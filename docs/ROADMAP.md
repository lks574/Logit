...
| P4 | ~~PostHog 계측~~ **구현 완료 2026-07-15 (키 발급 시 활성화)** | 1일 | `docs/ANALYTICS_PLAN.md` |
...
**상태**: ✅ 구현 완료(2026-07-15) — 배선 끝, **API 키 발급 + dev 빌드 재생성 시 활성화**. 상세·활성화 절차: `docs/ANALYTICS_PLAN.md`. 무키 상태에선 완전 no-op(지연 require)라 지금 도는 빌드에 무해. 웹 프리뷰에서 토글 UI·번들·콘솔 클린 확인. 실제 이벤트 수신은 키+네이티브 필요.

**구현**: `docs/ANALYTICS_PLAN.md` 그대로 (린 계측, opt-out 기본 토글, PII 금지). 반영:
...
- `stats_viewed`는 별도 이벤트 대신 화면뷰 추적으로 흡수(ponytail).
