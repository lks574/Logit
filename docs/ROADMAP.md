...
| ~~P4~~ | ~~PostHog 계측~~ **제외(2026-07-15, 도입 안 함)** | — | — |
...
## P4. PostHog 계측 — ❌ 제외 (2026-07-15)

한 번 배선했다가(커밋 e105291) **도입하지 않기로 결정, revert**. 필요 판단이 서면 그 커밋을 참고해 되살릴 수 있음.
아래 "버티컬 심화 게이트"는 계측 데이터 입력을 전제했으나, 계측 없이 진행할 경우 판단 근거는 직접 사용/피드백으로 대체.
