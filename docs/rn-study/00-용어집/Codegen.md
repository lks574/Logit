...
> [!abstract] 한 줄 정의
> TypeScript/Flow로 작성된 모듈·컴포넌트 스펙에서 iOS(Objective-C)와 Android(Java/Kotlin)용 인터페이스 코드를 **빌드 타임에** 자동 생성하는 [[New Architecture]]의 도구.

> [!info] iOS/AOS로 치면
> protobuf/gRPC의 코드 생성, OpenAPI 클라이언트 생성기, Sourcery나 KAPT/KSP 같은 발상. "계약을 한 곳(스키마)에 정의하고, 각 언어의 보일러플레이트는 기계가 만든다."
