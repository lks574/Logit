...
**한 줄 정의**: Swift/Kotlin **DSL**로 네이티브 모듈과 네이티브 뷰를 작성하는 Expo의 현대적 네이티브 확장 API. [[Turbo Module]]을 직접 작성하는 것보다 훨씬 적은 보일러플레이트로 같은 목표(JS ↔ 네이티브 연결)를 달성한다.

**iOS/AOS로 치면**: 네이티브 개발자가 **자기 언어로 RN 세계에 기능을 내보내는 공식 통로**다. 모듈 정의 자체가 Swift의 result builder / Kotlin DSL로 되어 있어, SwiftUI·Compose 스타일의 선언형 코드로 느껴진다.
