...
> [!abstract] 한 줄 정의
> 함수 컴포넌트에 상태([[State]])·생명주기·컨텍스트 같은 React 기능을 "걸어 쓸(hook into)" 수 있게 해주는 특수한 함수. 이름이 `use`로 시작한다 (`useState`, `useEffect`, `useRef` 등).

> [!info] iOS/AOS로 치면
> SwiftUI의 프로퍼티 래퍼(`@State`, `@Environment`, `.onAppear`)와 Compose의 `remember`/`LaunchedEffect` 계열. "선언형 UI 함수가 호출 간에 살아남는 상태와 부수효과를 갖게 하는 장치"라는 역할이 같다.
