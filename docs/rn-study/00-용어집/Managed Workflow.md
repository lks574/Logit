...
**한 줄 정의**: `ios/`·`android/` 네이티브 프로젝트를 개발자가 직접 관리하지 않고 **Expo가 설정(app.json)으로부터 생성·관리**하게 맡기는 개발 방식. 현재는 [[CNG]](Continuous Native Generation)라는 이름의 [[Prebuild]] 기반 모델로 정리되었다.

**iOS/AOS로 치면**: Xcode 프로젝트 파일(pbxproj)·Gradle 스크립트를 손으로 만지는 대신, 선언적 설정에서 **매번 새로 생성**하는 방식. "pbxproj를 직접 커밋하지 않고 Tuist/XcodeGen 정의에서 생성한다"에 가장 가깝다.
