...
**한 줄 정의**: 여러 패키지(앱, 공용 라이브러리, 디자인 시스템 등)를 **하나의 git 저장소**에서 함께 버전 관리·빌드하는 구조. JS 생태계에서는 패키지 매니저의 workspaces 기능으로 구현한다.

**iOS/AOS로 치면**: 한 Xcode 워크스페이스에 앱 타겟 + 로컬 Swift Package들을 두는 구조, 또는 하나의 Gradle 프로젝트에 `:app`, `:core`, `:feature-*` 모듈을 두는 멀티모듈 구성. "공용 코드를 별도 배포 없이 소스로 참조한다"는 목적이 같다.
