...
**한 줄 정의**: 컴포넌트가 소유하는, 시간에 따라 변하는 데이터. state가 바뀌면 React가 그 컴포넌트를 다시 렌더해서 UI를 새 값에 맞춘다.

**iOS/AOS로 치면**: SwiftUI의 `@State`, Compose의 `remember { mutableStateOf(...) }`. "이 값이 바뀌면 이 뷰를 다시 그려라"라고 프레임워크에 등록하는 지점이라는 점이 동일하다.
